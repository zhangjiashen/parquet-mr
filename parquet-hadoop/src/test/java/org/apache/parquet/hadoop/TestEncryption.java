...
public class TestEncryption {
  ...
  public void test() throws Exception {
    ...
    for (int encryptionMode = 0; encryptionMode < numberOfEncryptionModes; encryptionMode++) {
      ...
      // FileDecryptionProperties clone for 2nd decrypt test below on the same file
      FileDecryptionProperties fileDecryptionPropertiesClone = fileDecryptionProperties != null ?
        fileDecryptionProperties.deepClone(null) : null;

      if (encryptionMode == 1)
      System.out.println("xxx: " + encryptionMode);

      ParquetReader<Group> reader = ParquetReader.builder(new GroupReadSupport(), file).withConf(conf).withDecryption(fileDecryptionProperties).build();
      for (int i = 0; i < 1000; i++) {
        Group group = null;
        group= reader.read();
      ...
      // 2nd decrypt test: Validate readDictionary from within ParquetFileReader constructor.
      // Should decrypt and filter successfully.
      TestFileEncDecRetriever.setFileDecryptionProperties(fileDecryptionPropertiesClone);
      Configuration readerConf = new Configuration();
      readerConf.setBoolean(ParquetInputFormat.STATS_FILTERING_ENABLED, false);
      ParquetInputFormat.setFilterPredicate(readerConf, eq(intColumn("int32_field"), 30));
      ParquetReadOptions options = ParquetReadOptions.builder().withDecryption(fileDecryptionPropertiesClone).build();
      InputFile inputFile = HadoopInputFile.fromPath(file, readerConf);
      ParquetMetadata footer = ParquetFileReader.readFooter(inputFile, options, inputFile.newStream());
      ParquetFileReader parquetFileReader = new ParquetFileReader(
        readerConf, footer.getFileMetaData(), file, footer.getBlocks(), footer.getFileMetaData().getSchema().getColumns());
      parquetFileReader.close(); // done, validated constructor
    }
    enforceEmptyDir(conf, root);
  }
  ...
  /**
   * Simple FileEncDecryptorRetriever mock implemenation that returns globally set
   * test value (thus this is not thread safe, only intended to be used by this test).
   */
  public static class TestFileEncDecRetriever implements FileEncDecryptorRetriever {
    private static FileDecryptionProperties _fileDecryptionProperties;

    public static void setFileDecryptionProperties(FileDecryptionProperties fileDecryptionProperties) {
      _fileDecryptionProperties = fileDecryptionProperties;
    }

    public InternalFileEncryptor getFileEncryptor(Configuration conf, WriteContext writeContext) throws IOException {
      return null;
    }

    public FileEncryptionProperties getFileEncryptionProperties(Configuration conf, WriteContext writeContext) throws IOException {
      return null;
    }
    ...
    public FileDecryptionProperties getFileDecryptionProperties(Configuration conf) throws IOException {
      return _fileDecryptionProperties;
    }
  }
}
